...
class Vinculo(models.Model):  # Vínculo entre cliente e horário | Link between client and schedule
    ...
    cancelado = models.BooleanField(default=False) #models.BooleandField cria campos booleanos, definindo se o vínculo foi cancelado | BooleanField to indicate if the link is canceled
...
class BloqueioHorario(models.Model):
    ...
    def __str__(self):
        return f"{self.data} {self.hora_inicio.strftime('%H:%M')} - {self.hora_fim.strftime('%H:%M')} ({self.descricao})"
